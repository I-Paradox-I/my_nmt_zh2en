...
Options:
    ...
    --weight-decay=<int>                    L2 [default: 0.001]
"""
...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2, input_feed=True, label_smoothing=0.):
        ...
        # 编码器
        self.encoder_lstm = nn.LSTM(embed_size, hidden_size, num_layers=self.code_layers, bidirectional=True)
    ...
    def beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70) -> List[Hypothesis]:
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            prev_hyp_ids = top_cand_hyp_pos.floor_divide(len(self.vocab.tgt))
...
def train(args: Dict):
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=float(args['--lr']), weight_decay=float(args['--weight-decay']),  betas=(0.9, 0.999))
